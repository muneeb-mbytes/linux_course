# This command is used to clear the screen
clear
#-------------------------------------------------------------------------------------------
# This is used to store the path of current directory into WORKING_PATH
setenv WORKING_PATH `pwd`
#-------------------------------------------------------------------------------------------
# This is used to create a new directories in your current path.
#-------------------------------------------------------------------------------------------
echo $RED"Command: setenv variable 'value'"$NC
echo ""
echo $RED"Description:\
This command is used to create an environmental variable which can store a value or a command"\
$NC
echo ""
echo $RED"Expected output:"$NC
echo ""
#----------------------------------------------------------------------
setenv sample '10'
echo ""
echo $sample
unsetenv sample
cd $WORKING_PATH
#----------------------------------------------------------------------
echo $RED"-------------------------------------------------------------------------------------"\
$NC

echo $GREEN"please enter the command" $PURPLE"setenv sample '10'" $GREEN"<enter>"$NC
echo ""

come_here_1:
echo $PURPLE"Enter the command here:"$NC
echo ""
set userinput = `head -1`
echo ""

if("$userinput" == "setenv sample '10'") then
  #echo $userinput
  setenv sample '10'
  come_here_2:
  echo $PURPLE"Enter the command here:"$NC
  echo ""
  set userinput1 = `head -1`
  set compare = '$sample'
  if("$userinput1" == "$compare") then
    echo $GREEN"Actual output:"$NC
    echo ""
    echo $sample
  else
    echo $GREEN"Actual output:"$NC
    echo ""
    while (1)
      echo $GREEN"Entered wrong command:"$NC
      echo $GREEN"Try entering" $PURPLE"$sample" $GREEN"<enter>"$NC
      echo ""
      goto come_here_2
    end
  endif
else
  echo $GREEN"Actual output:"$NC
  echo ""
  while (1)
    echo $GREEN"Entered wrong command:"$NC
    echo $GREEN"Try entering" $PURPLE"setenv sample '10'" $GREEN"<enter>"$NC
    echo ""
    goto come_here_1
  end
endif



echo $RED"-------------------------------------------------------------------------------------"\
$NC

echo $PURPLE"Now you have learnt all about cd command \
and now you try doing remaining variations in cd."$NC
echo ""

cd $WORKING_PATH
# unsetting the environmental variables
unsetenv sample
